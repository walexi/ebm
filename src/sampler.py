...
"""
def ldmc(x: jax.Array, model_state: nnx.Module, step_size: int, steps: int) -> jax.Array:
    ...
    for sub_key in jax.random.split(key, steps):
        ...
        dx = jax.grad(lambda x_, p: -model_state.apply_fn({'params': p}, x_).sum(), argnums=0)(x, model_state.params)
...
class Sampler:
    ...
    def __init__(self, model_state: nnx.Module, shape: tuple, key: jnp.ndarray, buffer_size:int=32, sample_size: int=16):
        ...
        self.buffer = [jax.random.uniform(_key, (1,) + self.shape) for _key in jax.random.split(b_key, buffer_size)] # replay buffer to store samples shape (buffer_size, *shape)

    def generate(self, step_size: int, steps: int, method: algorithms=algorithms.LDMC):
        self.key, subkey = jax.random.split(self.key)
        if self.buffer:
            # 95% from buffer  
            from_buffer = jnp.vstack(random.choices(self.buffer, k=int(0.95*self.sample_size)))
            from_normal = jax.random.uniform(subkey, (self.sample_size-from_buffer.shape[0],) + self.shape)
            z = jnp.vstack([from_buffer, from_normal])
        else:
            # noise from uniform dist
            z = jax.random.uniform(subkey, (self.sample_size,) + self.shape)

        sample = self.sample(z, self.model_state, method, step_size, steps) # sample_size * *shape
    ...
    @staticmethod
    def sample(x: jnp.ndarray, model_state: nnx.Module, method: algorithms, step_size: int, steps: int):
