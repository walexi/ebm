...
class Trainer:
    ...
    def fit(self, train_loader: Any, val_loader: Any):
        for epoch, epoch_key in tqdm(
            ...
            running_loss = 0.0
            for (i, (batch_x, _)), _key in tqdm(
                zip(
                    enumerate(train_loader),
                    jax.random.split(epoch_key, len(train_loader)),
                ),
                leave=False,
                position=1,
                total=len(train_loader),
            ):
                z_plus = batch_x.reshape(
                    -1, self.hparams.image_size, self.hparams.image_size, 1
                )
                z_plus += (
                    jr.normal(_key, z_plus.shape) * 0.005
                )  # corrupt the original images with some random noise
                z_minus = self.sampler.generate(
                    self.hparams.step_size, self.hparams.steps
                )
    ...
    def _step(model_state, z, alpha, is_train=True):
        ...
        loss, grads = jax.value_and_grad(Trainer.compute_loss)(
            model_state.params, model_state, z, alpha
        )
        if is_train:
